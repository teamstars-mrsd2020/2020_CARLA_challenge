...
for i in range(len(data)):
    rgb, topdown, points, heatmap, heatmap_img, meta,target = data[i]
    ...
    # breakpoint()
    with torch.no_grad():
        # points_pred = net(torch.cat((topdown, heatmap), 0).cuda()[None],target.unsqueeze(0)).cpu().squeeze()
        # points_pred_flipped = net(torch.cat((topdown, heatmap_flipped), 0).cuda()[None],target.unsqueeze(0)).cpu().squeeze()
        points_pred = net(topdown.cuda()[None],target.unsqueeze(0)).cpu().squeeze()
        # points_pred_flipped = net(topdown.cuda()[None],target.unsqueeze(0)).cpu().squeeze()
